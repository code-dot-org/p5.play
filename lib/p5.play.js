...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  */
  ...
  /**
   * If set, do not update the sprite's position by the velocity vectory on
   * every update, unless sprite.moveByVelocityOneTick is set.
   *
   * @property disableVelocity
   * @type {Boolean}
   */
  this.disableVelocity = false;

  /**
   * If set at the beginning of a tick, update the sprite's position by the
   * velocity vector. This gets reset to false after the position is updated.
   *
   * @property moveByVelocityOneTick
   * @type {Boolean}
   */
  this.moveByVelocityOneTick = false;
  ...
  */
  this.update = function() {
    ...
    if(!this.removed)
    {
      ...
      if (!this.disableVelocity || this.moveByVelocityOneTick) {
